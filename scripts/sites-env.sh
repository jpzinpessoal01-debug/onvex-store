...
cd "${project_root}"
# ZIP extraction on Windows does not preserve executable bits. Shell scripts
# must therefore be launched explicitly through bash when this helper hands
# control back to a command such as build-verified.sh.
case "${1:-}" in
  *.sh) exec bash "$@" ;;
  *) exec "$@" ;;
esac
